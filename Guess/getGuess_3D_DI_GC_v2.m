...
if Options.KCFasinputstoExternalFunction
    if Options.preoptimizePelvisDOFs==1
        [pelvis_ty,pelvis_ty_v]=OptimizeForPelvis_ty(guess.Qs,guess.Qdots,Qs.colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence       
    elseif Options.preoptimizePelvisDOFs==2
        [pelvis_rtilt,pelvis_ty,pelvis_rtilt_v,pelvis_ty_v]=OptimizeForPelvis_ty_rtilt(guess.Qs,guess.Qdots,Qs.colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence
    end
else
    if Options.preoptimizePelvisDOFs==1
        [pelvis_ty,pelvis_ty_v]=OptimizeForPelvis_ty(guess.Qs,guess.Qdots,Qs.colheaders,jointi,GRF,N,F1,F2,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence        
    elseif Options.preoptimizePelvisDOFs==2
        [pelvis_rtilt,pelvis_ty,pelvis_rtilt_v,pelvis_ty_v]=OptimizeForPelvis_ty_rtilt(guess.Qs,guess.Qdots,Qs.colheaders,jointi,GRF,N,F1,F2,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence
    end
end
...
if strcmp(mode,'pred') %recompute velocities and accelerations consistent 
    % with the optimized knee_add and knee_ty
    spline_knee_add=spline(Qs.allinterpfilt(1:end,1),guess.Qs(:,jointi.knee_add.r));
    [aa,guess.Qdots(:,jointi.knee_add.r),...
            guess.Qdotdots(:,jointi.knee_add.r)] = ...
            SplineEval_ppuval(spline_knee_add,Qs.allinterpfilt(1:end,1),1);
    spline_knee_ty=spline(Qs.allinterpfilt(1:end,1),guess.Qs(:,jointi.knee_ty.r));
    [aa,guess.Qdots(:,jointi.knee_ty.r),...
            guess.Qdotdots(:,jointi.knee_ty.r)] = ...
            SplineEval_ppuval(spline_knee_ty,Qs.allinterpfilt(1:end,1),1);
end
...
guess.FTtilde = 0.001*ones(N+1,NMuscle);
guess.dFTtilde = 0.001*ones(N,NMuscle);

%% Arm activations
guess.a_a = 0.01*ones(N,nq.arms);
guess.e_a = 0.01*ones(N,nq.arms);
...
if Options.KCFasinputstoExternalFunction
    if Options.preoptimizePelvisDOFs==1
        [pelvis_ty_col, pelvis_ty_v_col]=OptimizeForPelvis_ty(guess.Qs_col,guess.Qdots_col,Qs.colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence        
    elseif    Options.preoptimizePelvisDOFs==2
        [pelvis_rtilt_col,pelvis_ty_col, pelvis_rtilt_v_col,pelvis_ty_v_col]=OptimizeForPelvis_ty_rtilt(guess.Qs_col,guess.Qdots_col,Qs.colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence
    end
else
    if Options.preoptimizePelvisDOFs==1
        [pelvis_rtilt_col,pelvis_ty_col, pelvis_rtilt_v_col,pelvis_ty_v_col]=OptimizeForPelvis_ty(guess.Qs_col,guess.Qdots_col,Qs.colheaders,jointi,GRF,N,F1,F2,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence
    elseif    Options.preoptimizePelvisDOFs==2
        [pelvis_rtilt_col,pelvis_ty_col, pelvis_rtilt_v_col,pelvis_ty_v_col]=OptimizeForPelvis_ty_rtilt(guess.Qs_col,guess.Qdots_col,Qs.colheaders,jointi,GRF,N,F1,F2,f_contactForce,Options,mode); % pre optimization of pelvis_ty and pelvis_tilt does not improve the convergence
    end
end
...
guess.Qdotdots_col=Qdotdots_spline_col.data(:,2:end);
if strcmp(mode,'pred') %recompute velocities and accelerations consistent 
    % with the optimized knee_add and knee_ty
    spline_knee_add_col=spline(Qs.allinterpfilt_col(1:end,1),guess.Qs_col(:,jointi.knee_add.r));
    [aa,guess.Qdots_col(:,jointi.knee_add.r),...
            guess.Qdotdots_col(:,jointi.knee_add.r)] = ...
            SplineEval_ppuval(spline_knee_add_col,Qs.allinterpfilt_col(1:end,1),1);
    spline_knee_ty_col=spline(Qs.allinterpfilt_col(1:end,1),guess.Qs_col(:,jointi.knee_ty.r));
    [aa,guess.Qdots_col(:,jointi.knee_ty.r),...
            guess.Qdotdots_col(:,jointi.knee_ty.r)] = ...
            SplineEval_ppuval(spline_knee_ty_col,Qs.allinterpfilt_col(1:end,1),1);
end
...
for i=1:size(Q,1)
    ...
    while any(abs(residual)>0.1)&(it<=120)
        ...
        q0(1)=q0_unsc(1)/0.1;
        q0(2)=(q0_unsc(2)-0.042)/0.007;
        [qopt,resnorm,residual] = lsqnonlin(@(x)fun2(x,newX(i,:),F2,jointi,KCF_in,nq,Options),q0,[-1 -1],[1 1],options_optim);
        
        
        it=it+1;
        if (it==30)&&Options.KCFasinputstoExternalFunction
            options_optim.Algorithm='interior-point';
            q0=[0 0];
            q0_unsc(1)=q0(1)*0.1;
            q0_unsc(2)=q0(2)*0.007+0.042;    
        elseif it==60&&Options.KCFasinputstoExternalFunction
            options_optim.Algorithm='trust-region-reflective';
            if i==1
                q0_unsc(1)=newX(i,2);
                q0_unsc(2)=newX(i,5);
            else
                q0_unsc(1)=newX(i-1,2);
                q0_unsc(2)=newX(i-1,5);
            end
            q0(1)=q0_unsc(1)/0.1;
            q0(2)=(q0_unsc(2)-0.042)/0.007;
        elseif it==80&&Options.KCFasinputstoExternalFunction
            options_optim.Algorithm='interior-point';
            if i==1
                q0_unsc(1)=newX(i,2);
                q0_unsc(2)=newX(i,5);
            ...
            q0(1)=q0_unsc(1)/0.1;
            q0(2)=(q0_unsc(2)-0.042)/0.007;
        elseif it>100&&Options.KCFasinputstoExternalFunction
...
function [pelvis_rtilt,pelvis_ty,pelvis_rtilt_v,pelvis_ty_v]=OptimizeForPelvis_ty_rtilt(Q,Qdots,colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode)
...
if   Options.useParFor_preopt==1
    parfor i=1:npoints
        ...
        cd(Options.pathExternalFunctions)
        ...
        options_optim = optimoptions('lsqnonlin', ...
            'MaxFunctionEvaluations', 10000,'MaxIterations',10000,'Display','none','UseParallel',false);
        [qopt,RESNORM,RESIDUAL,EXITFLAG,OUTPUT] = lsqnonlin(@(x)fun_preoptGRF(x,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode),q0,lb,ub,options_optim);
        ...
        [~, GRF_v_r(i,:), GRF_v_l(i,:)]=fun_preoptGRF(qopt,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode); %just for debug here
    ...
    for i=1:npoints
        ...
        q0=[Q(i,[jointi.pelvis.tilt  jointi.pelvis.ty]) Qdots(i,[jointi.pelvis.tilt  jointi.pelvis.ty])];
        options_optim = optimoptions('lsqnonlin', ...
            'MaxFunctionEvaluations', 10000,'MaxIterations',10000,'Display','none');
        [qopt,RESNORM,RESIDUAL,EXITFLAG,OUTPUT] = lsqnonlin(@(x)fun_preoptGRF(x,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode),q0,lb,ub,options_optim);
        ...
        pelvis_ty_v(i,1)=qopt(4);
        % all_output(i)=OUTPUT;
        % f_debug(:,i)=fun_preoptGRF(qopt,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode);
        [~, GRF_v_r(i,:), GRF_v_l(i,:)]=fun_preoptGRF(qopt,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode); %just for debug here
...
function [f, GRF_v_r, GRF_v_l]=fun_preoptGRF(x,X,F1,F2_skeletal,f_contactForce,jointi,GRF_exp,nq,Qsexp,Qdotsexp,Options,mode)

if Options.preoptimizePelvisDOFs==1
    X(:,jointi.pelvis.ty*2-1)=x(1);
    X(:,jointi.pelvis.ty*2)=x(2);
elseif Options.preoptimizePelvisDOFs==2
...
if strcmp(mode,'pred')
    ...
    if strcmp(mode,'pred')
        if Options.KCFasinputstoExternalFunction
            [Tj] = F2_skeletal([X';zeros(nq,1);zeros(6,1)]);
        else
            [Tj] = F2_skeletal([X';zeros(nq,1)]);
        end
    else
        if Options.KCFasinputstoExternalFunction
    ...
    if Options.preoptimizePelvisDOFs==1
        f=[f x(1)-Qsexp(jointi.pelvis.ty) x(2)-Qdotsexp(jointi.pelvis.ty)];
    elseif Options.preoptimizePelvisDOFs==2
        f=[f x(1)-Qsexp(jointi.pelvis.tilt) x(2)-Qsexp(jointi.pelvis.ty) x(3)-Qdotsexp(jointi.pelvis.tilt) x(4)-Qdotsexp(jointi.pelvis.ty)];
    else
        keyboard;
    end
...
function [pelvis_ty,pelvis_ty_v]=OptimizeForPelvis_ty(Q,Qdots,colheaders,jointi,GRF,N,F1,F2_skeletal,f_contactForce,Options,mode)
...
if   Options.useParFor_preopt==1
    parfor i=1:npoints
        ...
        q0=[Q(i,[jointi.pelvis.ty]) Qdots(i,[jointi.pelvis.ty])];
        options_optim = optimoptions('lsqnonlin', ...
            'MaxFunctionEvaluations', 10000,'MaxIterations',10000,'Display','none','UseParallel',false);
        [qopt,RESNORM,RESIDUAL,EXITFLAG,OUTPUT] = lsqnonlin(@(x)fun_preoptGRF(x,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode),q0,lb,ub,options_optim);
    
        pelvis_ty(i,1)=qopt(1);
        pelvis_ty_v(i,1)=qopt(2);
        % all_output(i)=OUTPUT;
        % f_debug(:,i)=fun_preoptGRF(qopt,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode);
        [~, GRF_v_r(i,:), GRF_v_l(i,:)]=fun_preoptGRF(qopt,X,F1,F2_skeletal,f_contactForce,jointi,GRFexp(i,:),nq,Q(i,:),Qdots(i,:),Options,mode);
    end
else
