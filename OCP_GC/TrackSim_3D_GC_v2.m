...
%% Muscle-driven simulations including mesh-based contact
    ...
    %Related to mesh-based contact model
        %Choose .dll
        if ~isfield(Options,'maxsmoothness')
            Options.maxsmoothness = 'MellowMax';
        end
    %Related on how the expression graph is treated
        Options.useSerialisedFunction=1; 

    %Related to parfor in pre-optimizations
        Options.useParFor_preopt=1;

       % ===== required fields =====
        reqFields = {'kInpress','kInCheckContacts','nfacesTib','nfacesFem','rad4Pairs'};
        if ~isfield(Options,'maxsmoothness'); Options.maxsmoothness = 'MellowMax'; end
        for f = 1:numel(reqFields)
            if ~isfield(Options, reqFields{f})
                error('Options.%s must be set by the caller.', reqFields{f});
            end
        end
        % In MellowMax, kInmaxpen is REQUIRED but may be numeric OR the string "Max"
        if strcmpi(Options.maxsmoothness,'MellowMax')
            if ~isfield(Options,'kInmaxpen') || isempty(Options.kInmaxpen)
                error('Options.kInmaxpen must be set in MellowMax mode (numeric or "Max").');
            end
        end


        
        Options.numpairs = [];   % keep as-is (legacy not used)
        Options.setLBKCF = 0;
    ...
    %Related to IG
        ...
        Options.preoptimizePelvisDOFs=1; %0 is considering all dofs as 
        % experimental data, 1 is preoptimizing pelvis ty, and 2 is 
        % preoptimizing pelvis tilt and ty with the goal to start with an
        % initial guess to match GRF closely
    ...
    % the optimization.
    if Options.useANNforKneeCont
        pathExternalFunctions_ANN = [pathRepo,'/ExternalFunctions_GC_ANN'];
        if Options.prunedANN
            suffix_typeKneeCont='_ANN_pruned';
        else
            suffix_typeKneeCont='_ANN';
        end
    else
        suffix_typeKneeCont=''; %empty is for collision detection algorithm
    end
    pathExternalFunctions = [pathRepo,'/ExternalFunctions_GC'];

    % Loading external functions. 
    % cd(pathExternalFunctions);
    Options.pathExternalFunctions=pathExternalFunctions;
    ...
    switch setup.derivatives
            ...
            if hessi == 1
                ...
                elseif Options.useContactModelwithTangentialForce0808
                    ...
                    elseif strcmp(Options.maxsmoothness,'MellowMax')
                        % --- normalize kInmaxpen so we can handle strings like "1e3" and "Max" ---
                    kmaxIsMax = false;
                    if (isstring(Options.kInmaxpen) || ischar(Options.kInmaxpen))
                        if strcmpi(Options.kInmaxpen,'Max')
                            kmaxIsMax = true;
                        else
                            tmp = str2double(Options.kInmaxpen);
                            if ~isnan(tmp)
                                Options.kInmaxpen = tmp;  % convert "1e3" -> 1000
                            end
                        end
                    end
                        % radius token
                    switch Options.rad4Pairs
                        case 0.5, radStr = 'rad05';
                        case 1,   radStr = 'rad1';
                        otherwise
                            error('Unsupported rad4Pairs=%g (use 0.5 or 1)', Options.rad4Pairs);
                    end
                    
                    % kmax token: accept numeric or "Max"
                    if kmaxIsMax
                        kmaxStr = 'Max';
                    else
                        switch Options.kInmaxpen
                            case 1e3, kmaxStr = '1e3';
                            case 1e4, kmaxStr = '1e4';
                            case 2e4, kmaxStr = '2e4';
                            otherwise
                                error('No DLL build for kInmaxpen=%g in MellowMax mode', Options.kInmaxpen);
                        end
                    end
                    
                    % kpress token: include 1e3 to match your DLL
                    switch Options.kInpress
                        case 1e3, kpressStr = '1e3';   % <-- add this
                        case 1e4, kpressStr = '1e4';
                        case 1e5, kpressStr = '1e5';
                        otherwise
                            error('No DLL build for kInpress=%g', Options.kInpress);
                    end
                    
                    % checkContact token
                    switch Options.kInCheckContacts
                        case 1e2, kcheckStr = '1e2';
                        case 1e3, kcheckStr = '1e3';
                        case 1e4, kcheckStr = '1e4';
                        otherwise
                            error('No DLL build for kInCheckContacts=%g', Options.kInCheckContacts);
                    end

                    % Build DLL names EXACTLY like yours
                    dllMain = sprintf( ...
                        'TrackSim_2_kneeCont_MellowMax_kmax%s_kpress%s_checkContact%s_%dx%d_%s%s.dll', ...
                        kmaxStr, kpressStr, kcheckStr, Options.nfacesTib, Options.nfacesFem, radStr, suffix_typeKneeCont);
                    
                    dllDbg  = sprintf( ...
                        'TrackSim_2_kneeCont_MellowMax_kmax%s_kpress%s_checkContact%s_%dx%d_%s_debug%s.dll', ...
                        kmaxStr, kpressStr, kcheckStr, Options.nfacesTib, Options.nfacesFem, radStr, suffix_typeKneeCont);
                    Options.dllMain=dllMain;
                    Options.dllDbg=dllDbg;
                        
                        % If this run uses KCF as inputs, load skeletal external too
                        if isfield(Options,'KCFasinputstoExternalFunction') && Options.KCFasinputstoExternalFunction
                            F2_skeletal = external('F','TrackSim_2_kneeCont_KCFasinput.dll');
                        end
                        
                        if Options.useANNforKneeCont
                            cd(pathExternalFunctions_ANN)
                            Options.pathExternalFunctions_ANN=pathExternalFunctions_ANN;
                        end
                        if ~exist(dllMain,'file')
                            error('DLL not found: %s (cwd: %s)', dllMain, pwd);
                        end
                        if ~exist(dllDbg,'file')
                            warning('Debug DLL not found: %s (continuing without it)', dllDbg);
                        end
                        
                        if Options.useSerialisedFunction
                            F2 = Function.load(strrep(dllMain,'dll','casadi'));
                        else
                            F2 = external('F', dllMain);
                        end
                        if exist(dllDbg,'file')
                            F2_debug = external('F', dllDbg);
                        end

        
                    else
                        error('Unknown Options.maxsmoothness = %s', Options.maxsmoothness);
                    end
                end
        ...
        case 'FD'
            % Deprecated finite-difference fallback
    ...
    %         JointMom_reskj=[JointMom_resk JointMom_resj];
        ...
        GRF_toTrackj=MX.sym('GRF_toTrackj',6,d); %resembles GRF.val.allinterp (except first column) at collocation points; first 3 right forces and then 3 left forces
    ...
    %     GRM_toTrackk=MX.sym('GRM_toTrackk',6); %resembles GRF.MorGF.allinterp (except first column) at collocation points
        GRM_toTrackj=MX.sym('GRM_toTrackj',6,d); %resembles GRF.MorGF.allinterp (except first column) at collocation points; first 3 right moments and then 3 left moments
    ...
    %                taken into account
            ...
            J2=J2+W.GRF*B(j+1)*(f_J6((Tj(GRFi.all,1)./scaling.GRF')-...
                        GRF_toTrackj(:,j)./scaling.GRF'))*h; % first 3 right forces and then 3 left forces
            J3=J3+W.GRM*B(j+1)*(f_J6((Tj(GRMi.all,1)./scaling.GRM')-...
                        GRM_toTrackj(:,j)./scaling.GRM'))*h; % first 3 right moments and then 3 left moments
            ...
            if Options.useJointResMom
                eq_constr{end+1} = Tj(jointi.knee_flex.r,1)-(T_knee_flex_r + ...
                    Tau_passj.knee_flex.r)- JointMom_resj(jointi.knee_flex.r,j)*scaling.JRM_res;
            else
                eq_constr{end+1} = Tj(jointi.knee_flex.r,1)-(T_knee_flex_r + ...
    ...
    %     guess=opti.debug.value(X_aux,opti.initial);
        ...
        if Options.nfacesTib==49
            nfacestib1=26;
            nfacestib2=23;
        elseif Options.nfacesTib==75
            nfacestib1=44;
            nfacestib2=31;
        elseif Options.nfacesTib==100
            nfacestib1=51;
            nfacestib2=49;
        elseif Options.nfacesTib==204
            nfacestib1=100;
            nfacestib2=104;
        else
            keyboard;
