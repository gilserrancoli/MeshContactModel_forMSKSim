...
% Date: 12/19/2018
%
function [muscle_spanning_joint_INFO,MuscleInfo] =  PolynomialFit_GC(MuscleData)
...
%     end
    ...
    threshold = 0.003; % maximal deviation from reference OpenSim data (in m)
    ...
    for m_nr=1:length(muscle_sel)
        ...
        internaldof_knee=any([index_dof_crossing_withknee==4 ...
            index_dof_crossing_withknee==5 index_dof_crossing_withknee==7 ...
            index_dof_crossing_withknee==8 index_dof_crossing_withknee==9]);
        ...
        index_dof_crossing_withoutknee = ...
            find(muscle_spanning_joint_INFO(muscle_index,[1:3 6 10:end])==1);
        ...
        while criterion_full_filled==0
            try
            [mat,diff_mat_q] = n_art_mat_3(q_all(:,index_dof_crossing_withoutknee), order);
            catch
                keyboard;
            end
            ...
            criterion_full_filled = ...
                lMT_error_rms<=threshold & max(dm_error_rms)<=threshold;
            if order==max_order
        ...
        DM_all_error(m_nr, index_dof_crossing_withknee) = dm_error_rms;
